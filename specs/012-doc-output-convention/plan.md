...
**Structure Decision**: existing layout, no new directories in this repository. The legacy BRD folder stays exactly
where it is (spec Clarification 2); the only new directories this change creates anywhere are `docs/adr/` and
`docs/brd/` inside *consumer* projects at run time.
