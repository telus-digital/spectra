...
- Q: What happens in a consumer project that already has `Docs/ADR/` or a legacy BRD folder from an earlier Spectra version?
...
- Q: Does Spectra's own repository migrate its own legacy BRD folder to the new location?
  → A: No. It stays put as a historical input cross-referenced from `specs/`; only newly generated BRDs land in the
  canonical folder. (Superseded on 2026-09-20: the folder was removed from this repository altogether.)
...
- **FR-016**: The extension version MUST bump to `1.6.0` in `spectra/extension.yml` and `catalog.json`, with a matching
  ...
  `spectra update`.
- **FR-017**: Spectra's own legacy BRD directory MUST be left exactly as it is. *(Superseded 2026-09-20 — the
  directory was removed from this repository; the requirement never governed consumer projects.)*
...
- **SC-003**: A repository-wide grep for `Docs/ADR` and `/brds` returns hits only in historical artifacts
  (`specs/003-brd-generator/`, `specs/012-doc-output-convention/`, the `CHANGELOG.md` entries that record the
