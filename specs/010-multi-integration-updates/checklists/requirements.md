...
- **The one marker was resolved with the user, not guessed.** FR-028's flag name was the single
  ...
  in the source requirement was resolved with a documented default in the Assumptions section instead of a
  marker.
...
- **Evidence for the problem statement is external to this spec.** The nine verified findings (F1–F9)
  that establish the dependency's behaviour live in `research.md` § Verified findings and are cited from
  there rather than restated here, keeping the spec free of dependency internals while leaving the evidence traceable for
  `/speckit.plan`.
