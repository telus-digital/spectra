...
**Purpose**: Establish the baseline and confirm the dependency still behaves as `research.md` § Verified
findings recorded, so later regressions are attributable.
...
- [X] T019 [P] [US1] Add a `RecordPrecedence` test class to `tests/test_health.py` asserting the per-integration manifests win over the project-level record: seed `.specify/integration.json` with `version` at `0.16.5` while one manifest reads `0.15.1`, and assert `Core agents` reports needs-updating and names that integration (FR-004, spec 010 finding F2). This is the regression the feature exists for and the sharpest instance of SC-002 — the project-level field is rewritten on any single upgrade, so an implementation that still reads it first must fail here
