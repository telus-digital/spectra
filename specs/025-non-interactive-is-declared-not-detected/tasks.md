...
- [X] T028a **Closed by the maintainer, 2026-09-20.** Run `test-strategy` with no arguments in the harness that
  reproduced the defect and confirm the questions are asked with no non-interactive announcement (SC-001); then run
  with `--non-interactive` and confirm the announcement fires and nothing is asked (SC-002). This could never be
  closed from a build — whether a given host's agent reaches the right conclusion is observable only by running it
  there, which is precisely how the defect was found — so it was held open for the reporter and is closed on their
  say-so, not on a passing suite.
