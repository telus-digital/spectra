...
| Edge cases identified | 12, carried over in full from the source requirement §6. |
| Scope bounded | In-scope and out-of-scope inherited from the source requirement §5; GitHub-only stated in Assumptions with GitLab, Bitbucket, and Azure DevOps explicitly excluded. |
...
implementation detail: the source requirement §5.2 defines the feature as GitHub-only, so the platform is a scope
...
The source requirement §13 lists nine open questions. All nine had a defensible default, so they were resolved as
