...
`research.md` § Verified findings (F3) was a clean return to the original values, so the expected outcome is that this
...
1. **`specs/010-multi-integration-updates/spec.md` § Out of scope** — "Changing the project's default integration. Not as an
...
2. **The same section** — "Scaffolding or registering commands for non-default integrations… Spectra reports
...
The reasoning behind the reversal is recorded in `specs/011-integration-coverage/spec.md` and rests on findings F2–F4, which did not
exist when spec 010 was written.
