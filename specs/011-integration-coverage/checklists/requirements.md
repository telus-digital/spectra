...
- **No markers were raised, and that is a decision rather than an omission.** All six open questions in
  the source requirement were resolved with documented defaults in the Assumptions section. The one that came
...
- **This feature supersedes a published boundary.** `specs/010-multi-integration-updates/spec.md` and
  `specs/010-multi-integration-updates/spec.md` § Out of scope record that the project's default integration is changed "not as an end state and not
  ...
  `specs/011-integration-coverage/spec.md` — so the two specs do not silently disagree, the same treatment features 009 and 010
...
- **The overwrite authorization must stay unreachable.** FR-049 restates a guarantee the previous feature
  ...
  need it (finding F4) and must not acquire a second route to it. This is the single highest-risk item for
...
- **Evidence for the problem statement is external to this spec.** The nine verified findings (F1–F9)
  ...
  restoration of configuration after a rotation — live in `research.md` § Verified findings, reproduced
  against Spec Kit CLI 0.16.5. They are cited from there rather than restated here, keeping the spec free of dependency
