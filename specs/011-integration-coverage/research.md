...
recorded once, below, as findings **F1–F9**. They are cited by the decisions, not restated.

## Verified findings (F1–F9)
...
| F4 | Activation with **locally modified** shared infrastructure does **not** fail: the modified file is preserved, a warning names it, and the command succeeds. | Reproduced — a deliberately edited shared script was reported as preserved and the activation exited successfully. | Coverage needs no overwrite authorization and cannot discard a team's edits. This is materially safer than the upgrade path spec 010 had to gate. |
...
| F8 | Upgrading a **non-default** integration deliberately skips extension re-registration. | Source inspection, with an upstream issue cited in that code. | The spec 010 update path cannot repair coverage as a side effect; coverage must be an explicit step. |
| F9 | The set of installed integrations is the project's **recorded list**; a per-integration record on disk is not by itself membership, and the shared-infrastructure record is not an integration. | Inherited from spec 010 finding F8, unchanged. | Enumeration must read the recorded list, never the directory. |
...
observed behaviour in the probe project was a clean return to the original values (F3), so the expected
