...
- Q: The source requirement's BR-18 requires "nothing pre-selected". Does this change need it amended?
  → A: No. It was a historical input cross-referenced from `specs/`, Status **Draft** version 0.1.0, last updated
  2026-08-17, and predates specs 015 and 027. It records what was asked for then. The live requirement is this
  spec.
...
- **FR-012a**: Historical records MUST keep it. `spectra/CHANGELOG.md`'s existing entries and
...
- **SC-006**: `grep -rn "pre-selected"` over `spectra/commands/`, `spectra/README.md`, `docs/index.html`,
  `AGENTS_LIST.md`, and `README.md` returns nothing — and the same grep over `spectra/CHANGELOG.md` and
