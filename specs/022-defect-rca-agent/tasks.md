...
- [X] T080 Open the pull request for branch `022-defect-rca-agent` with `speckit.spectra.create-pr` — the extension's own `after_implement` hook — and confirm the body states the 1.15.0 bump, the new `spectra/commands/defect-rca.md` and `spectra/templates/defect-rca-template.md`, the four decisions from T059, and the three departures from `the source requirement` with a pointer to the plan section that argues them.
  ...
  > the four decisions from T059, and the three departures from the source requirement — plus T075
