...
### Changed
- **The packaged `TRADEMARK.md` no longer points at a closed issue tracker.** Its Questions section
  told a reader to open an issue on the repository. Issues are disabled there and the repository has a
  single owner and maintainer who accepts no outside contributions, so the pointer sent people at a
  404 and implied a channel that does not exist. It now states that plainly and leaves the terms above
  it as the complete statement of what the Apache-2.0 grant withholds.
- **A changelog reference to a deleted source document was rewritten.** The 1.18.0 entry cited the
  August requirements document behind `review-pr` by path; that document was removed from the
  repository, and the entry now cites `specs/008-review-pr/`, which carries the same record.

No command, template, or agent behaviour changed. The version moves because the packaged bytes did.
...
- **`speckit.spectra.review-pr` now proposes blockers and majors instead of pre-selecting nothing.** A
  ...
  **The published disclosure line changed with it.** It claimed *"every finding below was individually
  ...
  never appear there, in the body or on a line, exactly as before. The August source requirement's BR-18
  still read "nothing pre-selected"; it was a historical input, not a live requirement, and recorded
  what was asked for then rather than what ships now. `specs/008-review-pr/` carries the same record.
