...
## 4. `specs/` — published content review

**Finding: overwhelmingly clean. The one derivation it flagged was resolved on 2026-09-20.**

**Update 2026-09-20.** This section originally covered `brds/` as well — this repository's own
Business Requirements Documents, the historical inputs to `specs/`. That folder has since been removed
from the repository, and every reference to it from `specs/`, `CONTRIBUTING.md` and
`spectra/CHANGELOG.md` was rewritten. The text below is kept as the record of what was audited at
commit `1b0cad6`, with the one disclosure it identified now closed — see 4a.

Twenty-five spec directories (182 files) are published publicly. Every one describes Spectra's own
development.

Systematic greps found **none** of the following anywhere in the published artifacts:
...
### 4a. The one exception — an internal source document — resolved

The Flaky Test Detector's source requirement recorded its own input as an internal TELUS Digital QE
Practice reference document, and named that document's filename. It was corroborated at
`specs/018-flaky-test-detector/spec.md:14`, which describes scoping down "the TELUS Digital QE Practice
reference document".

This was the only place where internal, non-public TELUS Digital material demonstrably fed a published
artifact. The derived spec tree and the shipped 27 KB `spectra/commands/flaky-test-detector.md` both
descend from it, and the internal document's filename being public was itself a small disclosure.

**Resolved 2026-09-20.** The source requirement was deleted and the repository history squashed to a
single commit, so the filename appears neither in the working tree nor in `main`'s history. The
surviving reference in `specs/018-flaky-test-detector/spec.md:14` names the practice, not the
document. Two caveats, stated so the closure is not overstated: the pre-squash objects stay reachable
through the seven release tags, which were deliberately preserved, and merged pull-request refs
(`refs/pull/N/head`) are not rewritten by a force-push. The filename is removed from the current tree
and from branch history, not erased from GitHub.

**One question still belongs to the owner:** was the source document cleared for derivative
publication? A scoped-down derivative of an internal work is still a derivative of it — and if it was
cleared, the derivation may warrant disclosure in `NOTICE`.
...
|---|---|---|---|
| 1 | Was the internal QE Practice document cleared for derivative publication? (its filename is no longer published) | Repo owner + legal | [4a](#4a-the-one-exception--an-internal-source-document--resolved) |
