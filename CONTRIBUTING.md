...
Spectra has a **single owner and maintainer**, [`@alibahaloo`](https://github.com/alibahaloo) (see
[`.github/CODEOWNERS`](.github/CODEOWNERS)). This repository accepts no outside contributions: Issues
and Discussions are disabled, and pull requests from outside are not accepted. Forking is permitted
under Apache-2.0 — see [`TRADEMARK.md`](TRADEMARK.md) for the rebranding a fork requires.

These are the maintainer's own notes, kept public so the build, packaging, and release rules are
...
is itself how the published extension gets exercised. A new capability is a **new command file under
...
**1. Nobody else contributes.** Spectra is written and maintained by TELUS Digital through a single
maintainer. Code contributions from outside are not accepted, in any form. External forks are
permitted under Apache-2.0, but changes cannot be contributed back.
...
**There is no public inbound channel.** Issues and Discussions are disabled, so there is nowhere to
file a bug report, a feature request, or a question. The one exception is a security vulnerability:
report it privately through GitHub private vulnerability reporting, per [`SECURITY.md`](SECURITY.md).
...
Spec Kit does not review or support extension code — this repository owns it. Pin a realistic
